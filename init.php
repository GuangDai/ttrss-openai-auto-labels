...
class OpenAI_Auto_Labels extends Plugin {
    ...
    function about() {
        return array(1.0,
            "Automatically assign up to 5 labels to articles using OpenAI API",
            "fangd123");
    }
    ...
    - Aim for 3-5 tags total (combination of existing and new tags).
    ...
    EOT;
        ...
        try {
            if (!isset($response_data['choices'][0]['message']['content'])) {
                Logger::log(E_USER_WARNING, "OpenAI_Auto_Labels", "OpenAI API response format异常: " . json_encode($response_data));
                return array();
            }
            ...
            $final_tags = array_slice($all_tags, 0, 5);
            return $final_tags;
